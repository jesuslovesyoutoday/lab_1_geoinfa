...
def define_period():
	...
	input_start= input('Enter a start date (YYYY-MM-DD-HH-MM)  :   ')
	start_year, start_month, start_day, start_hour, start_minute = map(int, input_start.split('-'))
	start_date = datetime(start_year, start_month, start_day, start_hour, start_minute)

	input_finish = input('Enter a finish date (YYYY-MM-DD-HH-MM) :   ')
	finish_year, finish_month, finish_day, finish_hour, finish_minute = map(int, input_finish.split('-'))
	finish_date = datetime(finish_year, finish_month, finish_day, finish_hour, finish_minute)
	
	print(' ')

	utc_start = start_date - timedelta(hours=3)
	utc_finish = finish_date - timedelta(hours=3)
	
	return [utc_start, utc_finish];
...
def get_LK_coords():
	...
	distance_to_LK = Earth_radius + height
...
def SHIT_WHAT(LK_coords):
	...
	while (time[0] != time[1]):
		...
		if theta >= 0:
			...
			y_n = 0
			z_n = LK / LK_coords[2]
			nor = [x_n - LK_coords[0], y_n - LK_coords[1], z_n - LK_coords[2]]
			len_nor = (nor[0] ** 2 + nor[1] ** 2 + nor[2] ** 2) ** 0.5
			t = (LK - sk)/LK
			x_p = s_x + t * LK_coords[0]
			y_p = s_y + t * LK_coords[1]
			z_p = s_z + t * LK_coords[2]
			pr = [x_p - LK_coords[0], y_p - LK_coords[1], z_p - LK_coords[2]]
			len_pr = (pr[0] ** 2 + pr[1] ** 2 + pr[2] ** 2) ** 0.5
	
			phi = math.acos((nor[0] * pr[0] + nor[1] * pr[1] + nor[2] * pr[2]) / (len_nor * len_pr))
			phi = degrees(phi)
	
			x1 = nor[0]
			y1 = nor[1]
			z1 = nor[2]
			est = [y1 * LK_coords[2] - z1 * LK_coords[1],
	                   -(x1 * LK_coords[2] - LK_coords[0] * z1),
	                   x1 * LK_coords[1] - y1 * LK_coords[0]]
			len_est = (est[0] ** 2 + est[1] ** 2 + est[2] ** 2) ** 0.5
			phi1 = math.acos((est[0] * pr[0] + est[1] * pr[1] + est[2] * pr[2]) / (len_est * len_pr))
			phi1 = degrees(phi1)
		...
		thetas.append(theta)
		phis.append(phi)
		times.append(time[0] + timedelta(hours = 3))
	...
	for i in range(len(thetas)):
		if thetas[i] >= 0:
			cur_event_thetas.append(thetas[i])
			cur_event_phis.append(radians(phis[i]))
			cur_event_times.append(times[i])
		else:
			if cur_event_thetas:
